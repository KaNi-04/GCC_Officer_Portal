...
@Service
@Transactional
public class ReturnFromAuditService {
   ...
   private RestTemplate restTemplate;
    ...
    public ReturnFromAuditService(RestTemplate restTemplate, ObjectMapper objectMapper) {
    ...
    public List<Map<String, Object>> fetchReturnEmpfiles(Integer deptId,Integer file_cat) {
        String query = "SELECT * " +
                       "FROM pensioner_details " +
                       "WHERE file_status=2 AND dept_id=? AND file_category=? AND is_closed=0";
    ...
    public boolean updateEntryDetails(Map<String, Object> requestData) {
        ...
        int filestatus = getFileStatus.getFileStatus3Id();
        if (filestatus == 0) {
  ...
  //save the updatedfamilypensionReturn
		...
		public boolean saveFamilyPensionReturnHistory(List<Map<String, Object>> emp_details,String filemovedby) {
		    try {
		        // Loop through the list of employee details
		        for (Map<String, Object> emp : emp_details) {
		            // Extract individual fields from the map
