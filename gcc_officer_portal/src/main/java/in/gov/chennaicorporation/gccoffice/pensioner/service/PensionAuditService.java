...
@Service
@Transactional
public class PensionAuditService {
	 ...
	 @Autowired
	   ...
	   private RestTemplate restTemplate;
	    ...
	    public PensionAuditService(RestTemplate restTemplate, ObjectMapper objectMapper) {
	    ...
	    public List<Map<String, Object>> fetchDetailsForAudit(Integer file_cat) {
	    	String query = "SELECT * " +
                    "FROM pensioner_details " +
                    "WHERE file_category=? AND (file_status=1 OR file_status=3)  AND is_closed=0";
	    ...
	    public boolean updateEntryDetails(Map<String, Object> requestData) {
            ...
            int filestatus = getFileStatus.getFileStatus4Id();
            if (filestatus == 0) {
                System.out.println("File status not found. Aborting insertion.");
                return false;
            }
            
            String file_entry_type="AUDIT_CLEARED";
		...
		//gis
		public List<Map<String, Object>> fetchDetailsForrecivedGis(Integer file_cat) {
			 String query = "SELECT * FROM pensioner_details WHERE file_category = ? AND file_status = 7";
