...
public class createventService {
	...
	// "WHERE file_url LIKE ?";
	// String filenamePattern = "%" + originalFilename; // Match filename at the end
	// of the path
	...
	public List<Map<String, Object>> ViewDropdownEvent(String userId) {
		String sql = "SELECT * FROM create_event WHERE cby=? AND isactive=1 AND isdelete=0 ";
		return jdbcTemplate.queryForList(sql, userId);
	}

	public List<Map<String, Object>> ViewCommitteeDropdownEvent(String userId) {
		String sql = "SELECT req_id,event_name,event_date,DATE_FORMAT(event_date, '%d-%m-%Y') as formated_date "
				+ "FROM create_event  "
				+ "WHERE isactive=1 AND isdelete=0 AND cby=?";
		try {
			return jdbcTemplate.queryForList(sql, userId);
		} catch (Exception e) {
			// TODO: handle exception
	...
	public ResponseEntity<Map<String, Object>> saveVendorDetails(Map<String, Object> payload) {
		...
		try {
			...
			List<Map<String, Object>> existingPostponed = jdbcTemplate.queryForList(checkPostponedSQL, vendor_req_id);
	...
	private String sendWhatsappMsg(String mob_no, String status, String vendor_req_number) {
		...
		if ("Approved".equals(status)) {
			urlString = "https://media.smsgupshup.com/GatewayAPI/rest?userid=2000233507&password=h2YjFNcJ&send_to="
					+ mob_no
					+ "&v=1.1&format=json&msg_type=TEXT&method=SENDMESSAGE&msg=You+are+recognized+as+a+roadside+vendor.+Registration+number%3A+"
					+ vendor_req_number + ".&isTemplate=true&footer=GCC+-+IT+Cell";
		} else {
			urlString = "https://media.smsgupshup.com/GatewayAPI/rest?userid=2000233507&password=h2YjFNcJ&send_to="
					+ mob_no
					+ "&v=1.1&format=json&msg_type=TEXT&method=SENDMESSAGE&msg=Your+application+is+rejected+because+your+roadside+vendor+details+are+incorrect.+%0A%0ARegistration+Number%3A+"
					+ vendor_req_number + ".";
		}
	...
	// @Transactional
	// public ResponseEntity<Map<String, Object>> saveBulkVendorDetails(Map<String,
	// Object> payload) {
	//
	// Map<String, Object> response = new HashMap<>();
	//
	...
	// e.printStackTrace();
	// return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	// .body(Map.of("status", "Error", "message", "Batch insert failed: " +
	// e.getMessage()));
	// }
	//
	//
	...
	// e.printStackTrace();
	// return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	// .body(Map.of("status", "Error", "message", "Failed to fetch inserted IDs: " +
	// e.getMessage()));
	// }
	//
	//
	...
	// try {
	// jdbcTemplate.batchUpdate("UPDATE vendor_request_list SET uid_no=? WHERE
	// id=?", updateParams);
	// } catch (Exception e) {
	// e.printStackTrace();
	// return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	// .body(Map.of("status", "Error", "message", "Failed to update UID numbers: " +
	// e.getMessage()));
	// }
	// }
	//
	...
	public ResponseEntity<Map<String, Object>> saveBulkVendorDetails(Map<String, Object> payload) {
		...
		try {
			...
			List<Map<String, Object>> vendorList = (List<Map<String, Object>>) payload.get("vendorList");
			...
			for (Map<String, Object> vendor : vendorList) {
				...
				List<Map<String, Object>> existingPostponed = jdbcTemplate.queryForList(checkSQL, vendorReqId);
				...
				if ("Postponed".equalsIgnoreCase(status)) {
					...
					if (!postponedExists) {
						...
						String insertPostSQL = "INSERT INTO vendor_postponed_list (vdid, status, remarks, event_req_id, cby) "
								+
								"VALUES (?, ?, ?, ?, ?)";
						...
						String updatePostSQL = "UPDATE vendor_postponed_list SET status=?, remarks=?, event_req_id=?, updated_by=?, updated_date=NOW() "
								+
								"WHERE vdid=?";
				...
				String insertReqSQL = "INSERT INTO vendor_request_list (vdid, status, remarks, event_req_id, cby) " +
						"VALUES (?, ?, ?, ?, ?)";
				 ...
				 */
				if (postponedExists) {
					String updatePostSQL = "UPDATE vendor_postponed_list SET status=?, remarks=?, event_req_id=?, updated_by=?, updated_date=NOW(),isactive=0,isdelete=1 "
							+
							"WHERE vdid=?";
	...
	private List<Map<String, Object>> getvendingfinaldata(String requestId) {
		...
		String sql = "SELECT all_logs.vdid, " +
				"       all_logs.vc_status, " +
				"       all_logs.vc_remarks, " +
	...
	public Map<String, Object> checkVendorInVendingTables(Integer vdid) {
		...
		String sqlRequest = "SELECT status FROM vendor_request_list WHERE vdid=? AND isactive=1 ORDER BY id DESC LIMIT 1";
