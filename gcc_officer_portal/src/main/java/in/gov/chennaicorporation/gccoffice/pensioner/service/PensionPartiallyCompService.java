...
@Service
@Transactional
public class PensionPartiallyCompService {
    ...
    public PensionPartiallyCompService(RestTemplate restTemplate, ObjectMapper objectMapper) {
    ...
    public List<Map<String, Object>> fetchPensionPartialEmpfiles(Integer file_cat) {
        String query = "SELECT * " +
                       "FROM pensioner_details " +
                       "WHERE file_status=10  AND file_category=? AND is_closed=0";
...
//            System.out.println("Rows updated in zb: " + rowsUpdated3);
                ...
                filestatus = CheckFieldsforPension(tempId) ?  getFileStatus.getFileStatus11Id() :  getFileStatus.getFileStatus10Id();
                ...
                if (rowsUpdated2 > 0) {
                    ...
                    System.out.println("Rows updated in fpd: " + rowsUpdated);
...
//            });
    ...
    private boolean CheckFieldsforPension(String tempId) {
		
    	// Define the SQL query
    	 String sql = "SELECT " +
                 "CASE " +
                 "WHEN pb.benefit_pension = 'yes' AND pb.benefit_dcrg = 'yes' " +
                 "THEN 1 ELSE 0 " +
                 "END AS allYes " +
                 "FROM pension_benefits AS pb " +
                 "WHERE pb.temp_id = ? ";
