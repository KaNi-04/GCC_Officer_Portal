...
public class NulmService {
	...
	public void setDataSource(@Qualifier("mysqlNulmDataSource") DataSource dataSource) {
	 ...
	 public void assignOrderToEnrollments(int orderId, List<Integer> enrollmentIds) {
		    ...
		    if (orderDetails != null) {
		        ...
		        StringBuilder updateSql = new StringBuilder("UPDATE enrollment_table SET order_id = ?, appointed = 'Yes', facial_attendance = 'No', appointed_date = CURRENT_DATE WHERE enrollment_id IN (");

		        // Add placeholders for the IN clause based on the size of the enrollmentIds list
	 ...
	 public Map<String, Integer> getAppointedCounts(int orderId) {
		    ...
		    Map<String, Integer> counts = jdbcTemplate.queryForObject(enrollmentQuery, new Object[]{orderId}, (rs, rowNum) -> {
	 ...
	 public List<Map<String, Object>> getAllOrderDetails() {
	    ...
	    public String markAttendance(int employeeId, String markAttendance) {
	        ...
	        // Query to check the current status of check_in and check_out for the employee on the current date
	        ...
	        // Retrieve the first (and only) result since attendance is checked by employee ID and date
	        ...
	        if ("OUT".equalsIgnoreCase(markAttendance) && checkInStatus == 1 && (checkOutStatus == null || checkOutStatus == 0)) {
	 ...
	 public List<Map<String, Object>> getEnrollmentsById(int enrollmentId) {
		    ...
		    try {
		        // Query the database for enrollment details based on the specified conditions and enrollment_id
 	...
 	/*
 	public String saveOrderDetails(String order_number, Date order_date, int no_of_staffs, String order_description, String order_copy_url, String order_generated_by, String category, Date validity_date) {
 	...
 	public String saveOrderDetails(String order_number, Date order_date, int no_of_staffs, String order_description, String order_copy_url, String order_generated_by, String category, Date validity_date) {
	    ...
	    try {
	        ...
	        System.out.println("INSERT INTO order_details (order_number, order_date, no_of_staffs, order_description, order_copy_url, order_generated_by, category, created_date, validity_date, order_status, isactive, isdelete) " +
	                 "VALUES ('" + order_number + "', '" + order_date + "', '" + no_of_staffs + "', '" + order_description + "', '" + order_copy_url + "', '" + order_generated_by + "', '" + category + "', CURRENT_TIMESTAMP, " +
	                 validity_date + ", CASE WHEN " + validity_date + " >= " + order_date + " THEN 'Active' ELSE 'Expired' END, 1, 0)");
 	...
 	public int addSalaryDetails(int totalDaysPresent, int totalDaysOd, int totalDaysSalary, int totalDaysAbsent, String month, int year, int salaryAmount, 
	...
	return jdbcTemplate.update(sql, totalDaysPresent, totalDaysOd, totalDaysSalary, totalDaysAbsent, month, year, salaryAmount, salaryStatus, enrollmentId, groupId, wageId);
	}
 	...
 	public List<Map<String, Object>> getAttendanceWithSalary(String month, Integer year) {
	    StringBuilder sql = new StringBuilder("SELECT " +
	        ...
	        "sk.group_name " +
	        "FROM enrollment_table e " +
	        "LEFT JOIN attendance a ON e.enrollment_id = a.enrollment_id " +  // Changed JOIN to LEFT JOIN to include all enrollments
...
/*
 	public List<Map<String, Object>> getAttendanceWithSalaryByInchargeId(String month, Integer year, Integer incharge_id) {
 	    String sql = "SELECT e.enrollment_id, e.emp_id, COALESCE(e.name, '') AS name, YEAR(a.indatetime) AS year, "
 	...
 	public List<Map<String, Object>> getAttendanceWithSalary(String month, Integer year) {
	    ...
	    sql.append("GROUP BY e.enrollment_id, e.name, YEAR(a.indatetime), MONTHNAME(a.indatetime), sd.salary_status, sk.group_id, sk.group_name");
	...
	public List<Map<String, Object>> getSalaryReport(String month, int year, String salaryStatus,String loginId) {
	    ...
	    String sql = "SELECT " +
	                 ...
	                 "e.incharge_designation, " +  // Fetch incharge_designation from enrollment_table
	                 "YEAR(a.indatetime) AS year, " +
	                 ...
	                 "COUNT(a.odby) AS total_days_od, " +
	                 "(COUNT(a.inby) + COUNT(a.odby)) AS total_days_salary, " +
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1) AS salary_per_day, " +  // Fetch salary_per_day from staff_wages table
	                 "((COUNT(a.inby) + COUNT(a.odby)) * " +
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1)) AS total_salary, " +  // Calculate total salary
	                 "sd.salary_status, " +
	                 ...
	                 "LEFT JOIN suyaudavi_kuzhu sk ON e.group_id = sk.group_id " +  // Join with suyaudavi_kuzhu table on group_id
	                 "WHERE (a.inby IS NOT NULL OR a.leaveby IS NOT NULL OR a.odby IS NOT NULL) " +  // Removed unauthorizedleaveby
	                 "AND MONTHNAME(a.indatetime) = ? " +  // Filter by month
	                 "AND YEAR(a.indatetime) = ? "+  // Filter by year
	                 "AND (sd.salary_status = 'Initiated' OR sd.salary_status = 'Approved') " +   // Filter by year
	                 (salaryStatus != null && !salaryStatus.isEmpty() ? "AND sd.salary_status = ? " : "") +  // Optional filter by salary status
	                 ZoneWhare +" GROUP BY e.enrollment_id, e.name, e.incharge_name, e.incharge_designation, YEAR(a.indatetime), MONTHNAME(a.indatetime), salary_per_day, sd.salary_status, sk.group_id, sk.group_name";  // Group by new fields
 	...
 	// Retrieve zone access for the user
	    ...
	    String sql = "SELECT " +
	                 ...
	                 "MONTHNAME(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)) AS imonth, " +  // Use MONTHNAME to get month in words
	                 "COUNT(DISTINCT DATE(a.indatetime)) AS total_days_present, " +
	                 "COUNT(DISTINCT DATE(a.leavedatetime)) AS total_days_absent, " +  // Changed to total_days_absent
	                 ...
	                 "AND (sd.salary_status = 'Initiated' OR sd.salary_status = 'Approved') " +   // Filter by year
	                 (salaryStatus != null && !salaryStatus.isEmpty() ? "AND sd.salary_status = ? " : "") +  // Optional filter by salary status
	                 ZoneWhare +" GROUP BY e.enrollment_id, e.name, e.incharge_name, e.incharge_designation, iyear, imonth, salary_per_day, sd.salary_status, sk.group_id, sk.group_name";  // Group by new fields
 	...
 	*/
 	public List<Map<String, Object>> getSalaryReport(String month, Integer year, String salaryStatus, String groupName, String loginId) {
...
*/
 	...
 	public List<Map<String, Object>> getSalaryReport(String month, Integer year, String salaryStatus, String groupName) {
		String sql = "select en.emp_id, en.name, en.incharge_name, en.incharge_designation, sk.group_name, s.total_days_present, s.total_days_od, "
				+ "s.total_days_absent, s.total_days_salary, s.month as imonth,s.year as iyear, s.salary_status, s.salary_amount as total_salary, "
				+ "DAY(LAST_DAY(STR_TO_DATE(CONCAT(s.year, '-', s.month, '-01'), '%Y-%M-%d'))) AS total_days from salary_details s "
				+ "left join enrollment_table en on s.enrollment_id = en.enrollment_id "
				+ "left join suyaudavi_kuzhu sk on s.group_id = sk.group_id "
...
*/
 	/*
 	public List<Map<String, Object>> getSalaryReport(String month, int year, String salaryStatus ,String groupName, String loginId) {
 		// Retrieve zone access for the user
	    String ZoneWhere;
		String zoneAccessSQL = "SELECT zone FROM gcc_apps.login_mapping_user WHERE appuser_id = ?";
	    List<Map<String, Object>> getAccess = jdbcTemplate.queryForList(zoneAccessSQL, loginId);
	    ...
	    if (!zones.isEmpty()) {
 		...
 		String sql = "SELECT " +
	                 ...
	                 "MAX(DAY(LAST_DAY(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)))) AS total_days, " +  // Calculate total days in the month
	                 "(COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime))) AS total_days_salary, " +  // Calculate total present and OD days
	                 "(MAX(DAY(LAST_DAY(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)))) - " +  // Calculate total days absent
	                 "(COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime)))) AS total_days_absent, " +  // Calculate total days absent
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1) AS salary_per_day, " +  // Fetch salary_per_day from staff_wages table
	                 "((COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime))) * " +
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1)) AS total_salary, " +  // Calculate total salary
	                 "sd.salary_status, " +
	                 ...
	                 "AND MONTHNAME(a.indatetime) = ? " +  // Filter by month
	                 "AND YEAR(a.indatetime) = ? " +  // Filter by year
	                 "AND (sd.salary_status = 'Initiated' OR sd.salary_status = 'Approved') " +   // Filter by salary status
	                 (salaryStatus != null && !salaryStatus.isEmpty() ? "AND sd.salary_status = ? " : "") +
	                 (groupName != null && !groupName.isEmpty() ? "AND sk.group_name = ? " : "")+// Optional filter by salary status
	                 ZoneWhere +" GROUP BY e.enrollment_id, e.name, e.incharge_name, e.incharge_designation, iyear, imonth, salary_per_day, sd.salary_status, sk.group_id, sk.group_name";  // Group by new fields
	...
	*/
 	...
 	/*
	public List<Map<String, Object>> getAttendanceWithInitiatedSalary(String inchargeName, String month) {
	    String sql = "SELECT " +
	                 "e.enrollment_id, " +
	                 "e.emp_id, " +		 
	                 "e.name, " +
	                 ...
	                 "sd.salary_status " +
	                 "FROM enrollment_table e " +
	                 "JOIN attendance a ON e.enrollment_id = a.enrollment_id " +  // Join with attendance table
	                 "LEFT JOIN salary_details sd ON e.enrollment_id = sd.enrollment_id " +  // Left join with salary_details
	                 "AND YEAR(a.indatetime) = sd.year " +  // Match year
	                 "AND MONTHNAME(a.indatetime) = sd.month " +  // Match month name
	                 "WHERE (a.inby IS NOT NULL OR a.leaveby IS NOT NULL OR a.odby IS NOT NULL) " +  // Removed unauthorizedleaveby
	                 "AND e.incharge_name = ? " +  // Filter by incharge_name
	                 "AND MONTHNAME(a.indatetime) = ? " +  // Filter by month
	                 "AND sd.salary_status IN ('Initiated', 'Approved') " +  // Filter by salary status 'Initiated' or 'Approved'
	                 "GROUP BY e.enrollment_id, e.name, YEAR(a.indatetime), MONTHNAME(a.indatetime), salary_per_day, sd.salary_status";  // Group by required fields

	    List<Map<String, Object>> results = jdbcTemplate.queryForList(sql, inchargeName, month);
	    return results;
	}
		*/
 	/*
 	public List<Map<String, Object>> getAttendanceWithInitiatedSalary(String inchargeName, String month) {
	    String sql = "SELECT " +
	                 ...
	                 "AND (MONTHNAME(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)) = ? OR ? IS NULL) " + // Ensure month matches
	                 "GROUP BY e.enrollment_id, e.emp_id, e.name, " +
	                 "YEAR(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)), " + // Grouping by COALESCE year
	                 "MONTHNAME(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)), sd.salary_status";  // Grouping by COALESCE month

	    List<Map<String, Object>> results = jdbcTemplate.queryForList(sql, inchargeName, month, month);
	    return results;
	}
	*/
 	/*
 	public List<Map<String, Object>> getAttendanceWithInitiatedSalary(String inchargeId, String month) {
	    String sql = "SELECT " +
	                 "e.enrollment_id, " +
	                 "e.emp_id, " +         
	                 "e.name, " +
	                 ...
	                 "COUNT(DISTINCT DATE(a.indatetime)) AS total_days_present, " +
	                 "COUNT(DISTINCT DATE(a.oddatetime)) AS total_days_od, " +
	                 "MAX(DAY(LAST_DAY(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)))) AS total_days, " +  // Total days in the month
	                 "(COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime))) AS total_days_salary, " +
	                 "(MAX(DAY(LAST_DAY(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)))) - " +  // Total days in the month
	                 "(COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime)))) AS total_days_absent, " +  // Calculate total days absent
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1) AS salary_per_day, " +
	                 "((COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime))) * " +
	                 "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1)) AS total_salary, " +
	    ...
	    List<Map<String, Object>> results = jdbcTemplate.queryForList(sql, inchargeId, month, month);
	    return results;
	}
 	*/
 	/*
 	public List<Map<String, Object>> getAttendanceWithInitiatedSalary(String inchargeId, String month, String inchargeName, Integer zone) {
 	    StringBuilder sql = new StringBuilder("SELECT " +
 	            "e.group_id, " +
 	...
 	*/
 	/*
 	public List<Map<String, Object>> getAttendanceWithInitiatedSalary(String inchargeId, String month, String inchargeName, int zone) {
	    StringBuilder sql = new StringBuilder("SELECT " +
	            "e.group_id, " +
	            "(SELECT group_name FROM suyaudavi_kuzhu WHERE suyaudavi_kuzhu.group_id = e.group_id LIMIT 1) AS group_name, " +
	            "a.enrollment_id, " +
	...
	*/
	/*
	public List<Map<String, Object>> getFilteredSalaryDetails(String month, Integer year, String salaryStatus, String department, String loginId) {
	    StringBuilder sql = new StringBuilder("SELECT " +
	                     "e.incharge_name, " +
	                     "GROUP_CONCAT(e.enrollment_id) AS enrollment_ids, " + 
	                     "COUNT(e.enrollment_id) AS total_staff_count, " + 
	    ...
	    sql.append(" GROUP BY e.incharge_name, sd.month, sd.year, sd.salary_status, e.department ");
	    sql.append(" ORDER BY sd.year DESC, sd.month DESC");
	...
	*/
	...
	public List<Map<String, Object>> getFilteredSalaryDetails(String month, Integer year, String salaryStatus, String loginId) {
	    StringBuilder sql = new StringBuilder("SELECT " +
	                     "e.incharge_name, " +
	    		         "e.incharge_id, " +
	    		         "e.zone, " +
	 ...
	 // Extract zones from getAccess
	    ...
	    sql.append(" GROUP BY e.incharge_name, e.incharge_id, e.zone, sd.month, sd.year, sd.salary_status, e.department ");
	    sql.append(" ORDER BY sd.year DESC, sd.month DESC");
/*
	    try {
	        List<Map<String, Object>> result = jdbcTemplate.queryForList(sql.toString());
	...
	public List<Map<String, Object>> getZoneWiseReport(String month, Integer year, String salaryStatus, String groupName, String zone, String division) {
	...
	public List<Map<String, Object>> getAttendanceReport(String fromdate, String todate, String groupName, String zone, String division) {
		String sql = "select date_format(coalesce(att.indatetime, att.oddatetime, att.leavedatetime),'%d-%m-%Y') as attendance_date, et.emp_id, et.name, "
				...
				+ " (DATE(att.indatetime) BETWEEN STR_TO_DATE('"+fromdate+"', '%Y-%m-%d') AND STR_TO_DATE('"+todate+"', '%Y-%m-%d') "
				+ " OR DATE(att.oddatetime) BETWEEN STR_TO_DATE('"+fromdate+"', '%Y-%m-%d') AND STR_TO_DATE('"+todate+"', '%Y-%m-%d')"
				+ " OR DATE(att.leavedatetime) BETWEEN STR_TO_DATE('"+fromdate+"', '%Y-%m-%d') AND STR_TO_DATE('"+todate+"', '%Y-%m-%d')) "
...
//		        "((COUNT(DISTINCT DATE(a.indatetime)) + COUNT(DISTINCT DATE(a.oddatetime))) * " +
//		        "(SELECT sw.salary_per_day FROM staff_wages sw WHERE sw.isactive = 1 ORDER BY sw.effect_from_date DESC LIMIT 1)) AS total_salary, " +
//		        "IFNULL(sd.salary_status, '') AS salary_status, " +  // Modified salary_status to return an empty string
//		        "sk.group_id, " +
//		        "sk.group_name " +
//		        "FROM enrollment_table e " +
//		        "JOIN attendance a ON e.enrollment_id = a.enrollment_id " +
...
//		}
	...
	public List<Map<String, Object>> getAttendanceWithSalaryByInchargeId(String month, Integer year, Integer incharge_id) {
	    ...
	    sql.append("GROUP BY e.enrollment_id, e.name, YEAR(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)), MONTHNAME(COALESCE(a.indatetime, a.leavedatetime, a.oddatetime)), total_days, sd.salary_status, sk.group_id, sk.group_name");
	...
	*/
	...
	public List<Map<String, Object>> getConsolidatedAttendanceReport(String fromdate, String todate, String groupName, String zone, String division) { // newly added
        String sql = "select \n" +
...
public Map<String, List<String>> fetchAttendanceDates(String enrollmentId, String fromdate, String todate, String type) { //newly added
        ...
        if (type.equals("present")) {
            String presentQuery = "SELECT DISTINCT DATE_FORMAT(DATE(indatetime), '%d-%m-%Y') AS PresentDates " +
                    ...
                    "STR_TO_DATE(? , '%Y-%m-%d')";
            attendanceData.put("presentDates", jdbcTemplate.queryForList(presentQuery, String.class, enrollmentId, fromdate, todate));
            ...
            attendanceData.put("odDates", jdbcTemplate.queryForList(odQuery, String.class, enrollmentId, fromdate, todate));
            ...
            attendanceData.put("leaveDates", jdbcTemplate.queryForList(leaveQuery, String.class, enrollmentId, fromdate, todate));
	...
	public int addSalaryDetails(
		...
		return jdbcTemplate.update(sql, totalDaysPresent, totalDaysOd, totalDaysSalary, totalDaysAbsent, month, year, salaryAmount, salaryStatus, enrollmentId, groupId, wageId, inchargeID);
	}
	...
	public int updateSalaryDetails(
		...
		return jdbcTemplate.update(sql, totalDaysPresent, totalDaysOd, totalDaysSalary, totalDaysAbsent, month, year, salaryAmount, salaryStatus, enrollmentId, groupId, wageId, inchargeID, enrollmentId, month, year);
	}
}